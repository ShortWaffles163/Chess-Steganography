...
def decode(pgn_string: str, output_file_path: str):
    ...
    print(f"\n[Decoder B] Successfully decoded: {len(games)} game(s), {total_move_count} moves ({round(time() - start_time, 3)}s).")
    ...
    # Print to Shell
    try:
        print(f"[Decoder B] Decoded Message: {result.decode('utf-8')}")
    except:
        print(f"[Decoder B] Decoded Bytes: {result}")
